...
describe("Phase 3 Boundary — No Real Withdraw", () => {
  it("withdraw only appears in types/mock verifier/comments, not as executable code", () => {
    for (const { file, content } of libRunFiles) {
      ...
      const isAllowed = normalizedPath.includes("types.ts") || normalizedPath.includes("testnetRouteTypes") || normalizedPath.includes("testnetRouteSecurityGuard") || normalizedPath.includes("permissionVerifier") || normalizedPath.includes(".test.");
